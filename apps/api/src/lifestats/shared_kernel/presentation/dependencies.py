...
from lifestats.identity.infrastructure.models import AppSession, User
from lifestats.shared_kernel.domain.time import utc_now
from lifestats.shared_kernel.infrastructure.database import session_scope
...
async def current_principal(
    ...
    row = await db.scalar(
        select(AppSession).where(AppSession.token_hash == digest, AppSession.expires_at > utc_now())
    )
    if row is None:
...
async def require_csrf(
    ...
    row: AppSession = request.state.app_session
