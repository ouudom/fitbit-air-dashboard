...
namespace App\Domain\Health\OAuth;
...
final readonly class GoogleOAuthClient
{
    ...
    public function authorizationUrl(string $state): string
    {
        $query = http_build_query([
            'client_id' => config('lifestats.google.client_id'),
            'redirect_uri' => config('lifestats.google.redirect_uri'),
            'response_type' => 'code',
            'access_type' => 'offline',
            'prompt' => 'consent',
            'include_granted_scopes' => 'true',
            'scope' => trim((string) config('lifestats.google.scopes')),
            'state' => $state,
        ], '', '&', PHP_QUERY_RFC3986);

        return config('lifestats.google.auth_url').'?'.$query;
    }
    ...
    public function exchangeCode(string $code): array
    {
        return $this->requestToken([
            'code' => $code,
            'client_id' => config('lifestats.google.client_id'),
            'client_secret' => config('lifestats.google.client_secret'),
            'redirect_uri' => config('lifestats.google.redirect_uri'),
    ...
    {
        $response = $this->http->asForm()->timeout(30)->post((string) config('lifestats.google.token_url'), $fields);
